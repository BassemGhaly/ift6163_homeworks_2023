...
from roble.infrastructure import pytorch_util as ptu
from roble.infrastructure import utils
from roble.infrastructure.logger import Logger
...
from roble.infrastructure.gclr_wrapper import GoalConditionedEnv, GoalConditionedEnvV2
from roble.infrastructure.hrl_wrapper import HRLWrapper
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, all_logs):
        ...
        if self.logmetrics:
            ...
            for key, value in logs.items():
                rospy.logwarn('{} : {}'.format(key, value))
                self.logger.log_scalar(value, key, itr)
            self.logger.log_file(itr, logs)
            rospy.logwarn('Done logging...\n\n')
            self.final_log[itr] = logs
            self.logger.flush()